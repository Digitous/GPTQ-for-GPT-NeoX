...
def get_neox(model):
    ...
    from transformers import GPTNeoXModel, GPTNeoXTokenizerFast, GPTNeoXConfig, GPTNeoXForCausalLM, AutoModelForCausalLM#, GPTNeoXTokenizer, AutoTokenizer
...
def neox_pack(model, quantizers, wbits):
...
def load_quant(model, checkpoint, wbits):
    ...
    config = NEOXConfig.from_pretrained(model)
